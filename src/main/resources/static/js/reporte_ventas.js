// Reporte de Ventas - Funcionalidades JavaScript
const ReporteVentas = (function() {
    // Variables privadas
    let formFiltros;
    let radiosTipoDocumento;
    ...
    function init() {
        ...
        radiosTipoDocumento = document.querySelectorAll('input[name="tipoDocumento"]');
    ...
    function configurarEventos() {
        // Auto-submit al cambiar radios de tipo documento
        if (radiosTipoDocumento) {
    ...
    function eliminarFiltros() {
        ...
        // Resetear radios
        const radioTodos = document.getElementById('doc-todos');
        if (radioTodos) {
    ...
    function exportarDatos() {
        const ventas = document.querySelectorAll('#tablaVentas tbody tr');
        if (ventas.length === 0) {
        ...
        // Encabezados
        const headers = ['Código', 'Cliente', 'Tipo Documento', 'Nº Documento', 'Vendedor', 'Método Pago', 'Importe Total', 'Estado', 'Fecha', 'Hora'];
        csvContent += headers.join(",") + "\n";
        
        // Datos
        ventas.forEach(venta => {
            const cells = venta.querySelectorAll('td');
            const row = [];
    ...
    function actualizarTotalRegistros() {
        ...
        if (totalRegistros) {
            const ventas = document.querySelectorAll('#tablaVentas tbody tr:not([style*="display: none"])');
            totalRegistros.textContent = ventas.length;
        }
    }
    ...
    function mostrarMensaje(mensaje, tipo = 'info') {
        ...
        const mensajesPrevios = document.querySelectorAll('.mensaje-ventas');
        mensajesPrevios.forEach(msg => msg.remove());
        
        const alerta = document.createElement('div');
        alerta.className = `mensaje-ventas fixed top-4 right-4 z-50 px-4 py-3 rounded-lg shadow-lg ${tipo === 'error' ? 'bg-red-100 text-red-800 border border-red-200' : 'bg-green-100 text-green-800 border border-green-200'}`;
        alerta.innerHTML = `
